...
<body>
	...
	<a href="/socialsnacks/show/${snack.id}">Back to ${snack.name}</a>
	<a href="/socialsnacks/profile/${user.id}">Profile</a>

	<h1>Edit ${snack.name}</h1>
	
	<form:form action="/socialsnacks/edit/${snack.id}" modelAttribute="snack" method="POST">
		...
		<form:select path="flavor">
			<form:option value="${snack.flavor.id}" >${snack.flavor.name}</form:option>
			<c:forEach var="flavor" items="${flavor}">
				<c:if test="${snack.flavor.id != flavor.id}">
